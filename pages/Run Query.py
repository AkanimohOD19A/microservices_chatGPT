...
txt_file = st.selectbox('Select a file', filenames)
txt_content = open(os.path.join(txt_path, txt_file), 'r').read()


# Get user input
transcript = "what is chatGPT"
if st.selectbox("Would like to use a transcript",
                ('Yes, please use my transcript',
                 'No, in my own words')) == "No, in my own words":
    user_query = st.text_input(f"Enter query here, to exit enter :q", str(transcript))
else:
    user_query = st.text_input(f"Enter query here, to exit enter :q", str(txt_content))
